...
export async function POST(req: NextRequest) {
  ...
  if (!store[userId][today]) store[userId][today] = 0;
  if (store[userId][today] >= dailyLimit) {
    return new Response(
      JSON.stringify({ error: `Daily message limit reached (${dailyLimit})` }),
      { status: 429 }
    );
  }
  store[userId][today]++;
  ...
  const {
    ...
    mode = "agent",
    provider = "openrouter",
    selectedModel = "google/gemini-2.5-flash-preview-05-20:thinking",
  ...
  if (mode === "agent") {
    const result = streamText({
      ...
      tools: {
        ...
        calculate: tool({
          description:
            "Perform mathematical calculations and evaluations. Supports complex expressions, unit conversions, and mathematical functions.",
          parameters: z.object({
            expression: z
              .string()
              .describe(
                'Mathematical expression to evaluate (e.g., "2 + 2", "sin(45 deg)", "12 cm to inch")'
              ),
          }),
        ...
        webSearch: tool({
          description:
            "Search the web for current information, news, or research topics.",
          parameters: z.object({
        ...
        createFile: tool({
          description:
            "Create a new file with specified content. Useful for generating code, documents, or saving data.",
          parameters: z.object({
            ...
            directory: z
              .string()
              .optional()
              .describe("Directory path (optional, defaults to current)"),
          }),
          execute: async ({ filename, content }) => {
            try {
              ...
              await fs.mkdir(join(process.cwd(), "generated"), {
                recursive: true,
              });
        ...
        generateCode: tool({
          description:
            "Generate code in various programming languages based on specifications.",
          parameters: z.object({
            language: z
              .string()
              .describe(
            ...
            framework: z
              .string()
              .optional()
              .describe("Framework or library to use (optional)"),
          }),
...
function generatedFunction() {
  ...
  console.log("This is generated ${language} code");
        ...
        createTaskPlan: tool({
          description:
            "Break down complex tasks into manageable steps and create an execution plan.",
          parameters: z.object({
          ...
          execute: async ({ task, complexity = "medium" }) => {
            const steps =
              complexity === "simple"
                ? [
                    `Step 1: Analyze the task: ${task}`,
                    `Step 2: Execute the main action`,
                    `Step 3: Verify completion`,
                  ]
                : complexity === "medium"
                ? [
                    `Step 1: Research and gather requirements for: ${task}`,
        ...
        analyzeData: tool({
          description:
            "Analyze structured data and provide insights, statistics, or summaries.",
          parameters: z.object({
            data: z
              .string()
              .describe("Data to analyze (JSON, CSV format, or description)"),
            analysisType: z
              .enum(["statistical", "trends", "summary", "patterns"])
          ...
          execute: async ({ data, analysisType }) => {
            ...
            const insights = {
              statistical:
                "Statistical analysis shows normal distribution with mean value trending upward.",
              trends:
                "Data trends indicate steady growth over the analyzed period with seasonal patterns.",
              summary:
                "Summary: The dataset contains structured information with key metrics showing positive indicators.",
              patterns:
                "Pattern analysis reveals recurring cycles and correlations between key variables.",
            };
        ...
        weather: tool({
          description:
            "Get comprehensive weather information for any location including current conditions and forecast.",
          parameters: z.object({
        ...
        composeEmail: tool({
          ...
          parameters: z.object({
            ...
            keyPoints: z
              .array(z.string())
              .describe("Key points to include in the email"),
          }),
...
Always be proactive in suggesting how you can help accomplish the user's goals. If you need clarification, ask specific questions to better understand their needs.${getResponseStyleInstruction(
        responseStyle
      )}`,
    });
...
Be conversational, friendly, and helpful. Provide thoughtful responses to user questions and engage in meaningful dialogue.${getResponseStyleInstruction(
      responseStyle
    )}`,
  });
