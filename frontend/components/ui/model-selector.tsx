...
import { Bot, Search, Filter, SortAsc, ChevronDown, X, DollarSign, Clock, Zap, Brain, Star, Eye, Shield, ChevronRight, Check, ArrowUpRight, Cpu } from "lucide-react"
...
const getProviderIcon = (providerId: string) => {
  ...
  if (provider.includes('anthropic') || providerId.includes('claude')) return Anthropic
  if (provider.includes('google') || providerId.includes('gemini')) return Gemini
            ...
            <CardContent className="p-4 space-y-3 h-full flex flex-col">
              ...
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-1">
                  ...
                  <span className={cn("font-medium", getPriceColor(model.pricing?.prompt))}>
...
function ModelGrid({ models, selectedModel, onModelSelect, searchQuery, sortBy, filterProvider }: ModelGridProps) {
  const filteredAndSortedModels = useMemo(() => {
    ...
    return filtered  }, [models, searchQuery, sortBy, filterProvider])
  return (
...
function ModelSelectorContent({ 
  ...
  const providers = useMemo(() => {
    ...
    models.forEach(model => {
      const provider = model.id.toLowerCase()
      if (provider.includes('openai') || provider.includes('gpt')) providerSet.add('openai')
      else if (provider.includes('anthropic') || provider.includes('claude')) providerSet.add('anthropic')
      else if (provider.includes('google') || provider.includes('gemini')) providerSet.add('google')
      else if (provider.includes('mistral')) providerSet.add('mistral')
      else if (provider.includes('meta') || provider.includes('llama')) providerSet.add('meta')
...
export function ModelSelector({ selectedModel, onModelSelect, provider, children, onOpenChange }: ModelSelectorProps) {
